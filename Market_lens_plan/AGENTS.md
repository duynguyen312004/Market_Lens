...
Mọi AI coding agent hoặc lập trình viên phải đọc `docs/MARKETLENS_MASTER_PLAN.md` trước khi sửa code.
...
5. Forecast theo rule trong master plan; không dùng LLM forecast.
6. AI API chỉ viết report từ aggregate JSON; không nhận raw rows hoặc PII.
7. Chỉ hỗ trợ upload theo fixed CSV/XLSX template.
...
14. Làm theo phase trong master plan; không cắt chức năng feature-complete V1.
...
Một task chỉ hoàn tất khi code chạy, xử lý error/empty/loading thích hợp, tuân thủ ownership, không lộ secret và đã được test theo master plan.
